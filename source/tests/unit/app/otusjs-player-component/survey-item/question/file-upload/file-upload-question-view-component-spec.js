describe('file upload question view controller component', function () {
  var Mock = {};
  var controller;
  ...
  describe('the initialization', function () {
    ...
    it('should build an new component with values', function () {
      ...
      expect(controller.sentFiles).toEqual([].concat(Mock.itemData.data));
      expect(controller.view).toEqual(true);

      controller.downloadFile(0);
      controller.uploadConfig.callback(controller.sentFiles);
      Mock.scope.$root.$$phase = "$digest";
      controller.uploadConfig.callback(controller.sentFiles);
      controller.itemData.data.answer.value = null;
      controller.$onInit();
    });
